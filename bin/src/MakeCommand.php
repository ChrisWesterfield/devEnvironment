...
namespace MjrOne\Vagrant;

use MjrOne\Vagrant\Settings\JsonSettings;
use MjrOne\Vagrant\Settings\YamlSettings;
use MjrOne\Vagrant\Traits\GeneratesSlugs;
...
class MakeCommand extends Command
{
    ...
    protected function configure()
    {
        ...
        $this
            ->setName('make')
            ->setDescription('Install vagrant into the current project')
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('vagrant Installed!');
    }
    ...
    {
        return file_exists("{$this->basePath}/config.{$format}");
    }
    ...
    {
        $SettingsClass = YamlSettings::class;

        $filename = $this->exampleSettingsExists($format) ?
            "{$this->basePath}/config.{$format}.example" :
            __DIR__."/../resources/config.{$format}";
        ...
        $settings->updateIpAddress($options['ip'])
            ...
            ->save("{$this->basePath}/config.{$format}");
    }
    ...
    {
        return file_exists("{$this->basePath}/config.{$format}.example");
    }
    ...
    {
        copy("{$this->basePath}/config.{$format}", "{$this->basePath}/config.{$format}.example");
    }
    ...
    {
        if (file_exists("{$this->basePath}/config.yaml") ) {
            $output->writeln(
                '<error>WARNING! You have config.yaml configuration files</error>'
            );
        }
    }
}
